 ...
 * @author root
 */
public class Solve_paper implements Initializable {
    ...
    void btnfinalans(ActionEvent event){
        ...
        if(select_ans[row]==2){
            JOptionPane.showMessageDialog(null, "Sorry, You cann't change Answer ","Error ",JOptionPane.ERROR_MESSAGE);
        }
        else{
            if(row<1){
                ...
                if(cmbans.getValue()==null){
                    ...
                    try{
                        ...
                        String query="update marks_list set q"+row +"= ? where username = ? && paper =?"; 
                        //String query="update "+Student_after_login.backup_paper_name+" set ans=? where Q_no= "+row;
                        //    String query="update  2017_04_28__PPL_End set question =?, opt1=?, opt2=?, opt3=?, opt4=?, ans=? where Q_no= "+row;
                        pstmt = conn.prepareStatement(query);            
                        pstmt = conn.prepareStatement(query);
                        pstmt.setString(1, cmbans.getValue().toString());
                        pstmt.setString(2,first_page.logged_user);
                        pstmt.setString(3,Student_after_login.backup_paper_name);
                        pstmt.executeUpdate();
    ...
    public void initialize(URL url, ResourceBundle rb) {
       ...
       try{
           ...
           System.out.println(Student_after_login.backup_paper_name);
